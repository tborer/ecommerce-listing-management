...
2. Submit `https://<site>/sitemap.xml` under Sitemaps. It's generated by `web/app/sitemap.ts` (public pages only). When you add a public page, add it there; when a page's content really changes, bump its date (`LANDING_UPDATED` / `PRIVACY_UPDATED` in `web/lib/site.ts`).
